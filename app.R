...
#data.frame(table(Summary$Invasion.Status.Full,Summary$Habitat))

combine_names <- file_names[which(rows == 1747)]
#file_names[which(rows != 1747)]
...
state_names <- c("India","Andaman and Nicobar Islands","Andhra Pradesh","Arunachal Pradesh","Assam","Bihar","Chandigarh", 
...
#           height = 100,
  ...
  observeEvent(var2, {
    ...
    output$data <- renderUI({
        conditionalPanel(condition = "input.main1==`Tables`", 
                         ...
                         here  </a>.</p>"),
                         renderDT({ validate(need(input$main1=="Tables", message = FALSE)) 
 ...
 var4 <- renderText(input$cat) 
 var5 <- renderText(input$cv)
 toListen <- reactive({
   list(var3(),var4(),var5())
 })
  ...
  # Variable Charts ------
  var7 <- renderText(input$ch2)
  
  observeEvent(var7(),{
    output$varchart <- renderUI({
